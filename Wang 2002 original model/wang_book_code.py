...
class Simulation(object):
    ...
    def savespikes(self, filename_exc, filename_inh):
        ...
        np.savetxt(
            path.join(path.dirname(__file__), filename_exc),
            list(zip(self.model.mons['spikesE'].i,
                self.model.mons['spikesE'].t)),
            fmt='%-9d %25.18e',
            header='{:<8} {:<25}'.format('Neuron', 'Time (s)'))

        print("Saving inhibitory spike times to " + filename_inh)
        np.savetxt(
            path.join(path.dirname(__file__), filename_inh),
            list(zip(self.model.mons['spikesI'].i,
                self.model.mons['spikesI'].t)),
            fmt='%-9d %25.18e',
            header='{:<8} {:<25}'.format('Neuron', 'Time (s)'))

    def loadspikes(self, *args):
        return [np.loadtxt(path.join(path.dirname(__file__), filename)) for filename in args]
...
if __name__ == '__main__':
    ...
    plt.savefig(path.join(path.dirname(__file__), 'wang2002.pdf'))
